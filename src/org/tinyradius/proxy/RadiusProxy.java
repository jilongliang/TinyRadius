...
/**
 * This class implements a Radius proxy that receives Radius packets and
 * forwards them to a Radius server. You have to override the method
 * getRadiusProxyConnection() which identifies the Radius proxy connection a
 * Radius packet belongs to.
 */
public abstract class RadiusProxy extends RadiusServer {
	...
	/**
	 * This method must be implemented to return a RadiusEndpoint if the given
	 * packet is to be proxied. The endpoint represents the Radius server the packet
	 * should be proxied to.
	 * 
	 * @param packet
	 *            the packet in question
	 * @param client
	 *            the client endpoint the packet originated from (containing the
	 *            address, port number and shared secret)
	 * @return a RadiusEndpoint or null if the packet should not be proxied
	 */
	public abstract RadiusEndpoint getProxyServer(RadiusPacket packet, RadiusEndpoint client);

	/**
	 * Returns the proxy port this server listens to. Defaults to 1814.
	 * 
	 * @return proxy port
	 */
	...
	/**
	 * Sets the proxy port this server listens to. Please call before start().
	 * 
	 * @param proxyPort
	 *            proxy port
	 */
	...
	/**
	 * Handles packets coming in on the proxy port. Decides whether packets coming
	 * in on Auth/Acct ports should be proxied.
	 */
	protected RadiusPacket handlePacket(InetSocketAddress localAddress, InetSocketAddress remoteAddress,
			RadiusPacket request, String sharedSecret) throws RadiusException, IOException {
		...
		if (radiusServer != null) {
			...
			RadiusProxyConnection proxyConnection = new RadiusProxyConnection(radiusServer, radiusClient, request,
	...
	/**
	 * Sends an answer to a proxied packet back to the original host. Retrieves the
	 * RadiusProxyConnection object from the cache employing the Proxy-State
	 * attribute.
	 * 
	 * @param packet
	...
	protected void proxyPacketReceived(RadiusPacket packet, InetSocketAddress remote)
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("forward packet to " + client.getEndpointAddress().toString() + " with secret "
		...
		RadiusPacket answer = new RadiusPacket(packet.getPacketType(), packet.getPacketIdentifier(),
				packet.getAttributes());
		DatagramPacket datagram = makeDatagramPacket(answer, client.getSharedSecret(),
				client.getEndpointAddress().getAddress(), client.getEndpointAddress().getPort(),
				proxyConnection.getPacket());
	...
	/**
	 * Proxies the given packet to the server given in the proxy connection. Stores
	 * the proxy connection object in the cache with a key that is added to the
	 * packet in the "Proxy-State" attribute.
	 * 
	 * @param packet
	 *            the packet to proxy
	 * @param proxyCon
	...
	/**
	 * Cache for Radius proxy connections belonging to sent packets without a
	 * received response. Key: Proxy Index (String), Value: RadiusProxyConnection
	 */

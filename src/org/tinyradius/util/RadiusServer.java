...
public abstract class RadiusServer {
	...
	/**
	 * Returns the shared secret used to communicate with the client with the passed
	 * IP address or null if the client is not allowed at this server.
	 * 
	 * @param client
	...
	public RadiusPacket accountingRequestReceived(AccountingRequest accountingRequest, InetSocketAddress client)
			throws RadiusException {
		RadiusPacket answer = new RadiusPacket(RadiusPacket.ACCOUNTING_RESPONSE,
				accountingRequest.getPacketIdentifier());
	...
	/**
	 * Returns the duplicate interval in ms. A packet is discarded as a duplicate if
	 * in the duplicate interval there was another packet with the same identifier
	...
	/**
	 * Sets the duplicate interval in ms. A packet is discarded as a duplicate if in
	 * the duplicate interval there was another packet with the same identifier
	 * originating from the same address.
	 * 
	...
	/**
	 * Returns the IP address the server listens on. Returns null if listening on
	 ...
	 * Defaults to null, meaning listen on every local address (wildcard address).
	 * 
	 * @param listenAddress
	...
	/**
	 * Listens on the auth port (blocks the current thread). Returns when stop() is
	 * called.
	 * 
	 * @throws SocketException
	...
	/**
	 * Listens on the acct port (blocks the current thread). Returns when stop() is
	 * called.
	 * 
	 * @throws SocketException
	 * @throws InterruptedException
	...
	protected void listen(DatagramSocket s) {
		DatagramPacket packetIn = new DatagramPacket(new byte[RadiusPacket.MAX_PACKET_LENGTH],
	...
	protected DatagramPacket makeDatagramPacket(RadiusPacket packet, String secret, InetAddress address, int port,
			RadiusPacket request) throws IOException {

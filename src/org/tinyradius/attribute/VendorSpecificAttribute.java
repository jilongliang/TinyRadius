...
public class VendorSpecificAttribute extends RadiusAttribute {
	...
	/**
	 * Constructs an empty Vendor-Specific attribute that can be read from a Radius
	 * packet.
	 */
	...
	public void addSubAttribute(String name, String value) {
		...
		if (type.getVendorId() != getChildVendorId())
			throw new IllegalArgumentException(
					"attribute type '" + name + "' does not belong to vendor ID " + getChildVendorId());
	...
	/**
	 * Returns a sub-attribute of the given type which may only occur once in this
	 * attribute.
	 * 
	 * @param type
	 *            sub-attribute type
	 ...
	 *             if there are multiple occurences of the requested sub-attribute
	 *             type
	 */
	...
	public void readAttribute(byte[] data, int offset, int length) throws RadiusException {
		...
		/*
		 * int vendorId = (data[offset + 2] << 24 | data[offset + 3] << 16 | data[offset
		 * + 4] << 8 | ((int)data[offset + 5] & 0x000000ff));
		 */

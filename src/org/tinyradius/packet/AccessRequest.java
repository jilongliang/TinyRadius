...
public class AccessRequest extends RadiusPacket {
	...
	/**
	 * Constructs an Access-Request packet, sets the code, identifier and adds an
	 * User-Name and an User-Password attribute (PAP).
	 * 
	 * @param userName
	 *            user name
	 * @param userPassword
	...
	/**
	 * Selects the protocol to use for encrypting the passphrase when encoding this
	 * Radius packet.
	 * 
	 * @param authProtocol
	...
	/**
	 * Verifies that the passed plain-text password matches the password (hash) send
	 * with this Access-Request packet. Works with both PAP and CHAP.
	 * 
	 * @param plaintext
	...
	protected void encodeRequestAttributes(String sharedSecret) {
		...
		if (getAuthProtocol().equals(AUTH_PAP)) {
			byte[] pass = encodePapPassword(RadiusUtil.getUtf8Bytes(this.password),
					RadiusUtil.getUtf8Bytes(sharedSecret));
